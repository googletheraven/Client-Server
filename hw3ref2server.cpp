...
//creates vector of Timed_Location objects
std::vector<Timed_Location> hw3_TL_vector;
//creates map of strings onto timed locations
...
(const Json::Value& location_jv)
{
  ...
  time_t ticks; 
  ticks = time(NULL);
  struct std::tm * my_tm_ptr = gmtime(&ticks); // creates time address for time since timer started

  char buffer[128];
  bzero(buffer, 128);
  std::strftime(buffer, 32, "%Y-%m-%dT%H:%M:%S", my_tm_ptr); //places my_tm_ptr into printable format
  printf("at %s\n", buffer);
  ...
  if ((location_jv["identity"].isNull() != true) &&
      ...
      (location_jv["traces"].isArray() == true))
    {
      ...
      //if not at end of location array
...
(const Json::Value& question_jv)
{
  ...
  //Timed_Location lastPoint = hw3_TL_vector[0];
  int index = 0;
  ...
  if (hw3_TLID_map.find(id_str) != hw3_TLID_map.end())
      ...
      for (i = 0; i < hw3_TL_vector.size(); i++)
	{
	  ...
	  if(time_diff > 0 && (jvt_question - hw3_TL_vector[index].time > time_diff))
	  {
		//(lastPoint) = (hw3_TL_vector[i]);
		index = i;
	  }
	  //std::cout << "the difference is " << time_diff << std::endl;
	  //std::cout << std::endl;
	}
      std::cout << "Smallest time diff: " << *(hw3_TL_vector[index].dump2JSON()) << std::endl;
      return *(hw3_TL_vector[index].dump2JSON());
    }
  else
    {
      std::cout << "question JSON content error" << std::endl;
